...
bool Graph::setState(const GraphState new_state)
{
    ...
    if (new_state == GraphState::kSuccess)
    {
        ...
        LM_LOG_INFO() << "Completed the request for run target" << getRequestedState() << "in" << timeDiff.count()
                      << "ms";
    }
...
void Graph::finalizeTransitionSuccess()
{
    ...
    {
        const IdentifierHash state = getRequestedState();
...
IdentifierHash Graph::getRequestedState()
{
    std::lock_guard<std::mutex> lock(requested_state_mutex_);
