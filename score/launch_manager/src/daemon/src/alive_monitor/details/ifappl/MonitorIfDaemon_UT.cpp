...
class MonitorIfDaemonTest : public ::testing::Test
{
  ...
  public:
    ...
    [[nodiscard]] timers::NanoSecondType mockClock()
    {
    ...
    [[nodiscard]] timers::NanoSecondType mockClockOffset() const
    {
        return time_ + 50U;
    }

    /// @brief Get the time @c count mockClock() calls from now
    [[nodiscard]] timers::NanoSecondType mockClockFuture(int count) const
    {
        return time_ + (kTimeStep * count);
    }
};
