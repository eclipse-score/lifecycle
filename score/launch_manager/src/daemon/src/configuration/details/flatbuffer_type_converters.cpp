    ...
    const fb::RestartAction* ra)
{
    ...
    auto delay_before_restart = requireScalarValue(ra->delay_before_restart_ms(), "RestartAction::delay_before_restart_ms");
...
score::cpp::expected<DeploymentConfig, IConfigLoader::Error> convertDeploymentConfig(const fb::DeploymentConfig* fb_dc)
{
    ...
    if (fb_dc != nullptr)
    {
        ...
        auto shutdown_timeout = requireScalarValue(fb_dc->shutdown_timeout_ms(), "DeploymentConfig::shutdown_timeout_ms");
...
score::cpp::expected<RunTargetConfig, IConfigLoader::Error> convertRunTarget(const fb::RunTarget* fb_rt)
{
    ...
    if (fb_rt != nullptr)
    {
        ...
        auto transition_timeout = requireScalarValue(fb_rt->transition_timeout_ms(), "RunTarget::transition_timeout_ms");
