...
/// @brief Class representing connection to Launch Manager that is used to request Run Target activation (or other
/// operations).
/// @note ControlClient opens communication channel to Launch Manager (e.g. POSIX FIFO). Each Process that intends to
/// perform state management, shall create an instance of this class and it shall have rights to use it.
///
class ControlClient final
{
  public:
    ...
    ///
    /// @param[in] runTargetName name of the Run Target that should be activated. Launch Manager will deactivate the
    /// currently active Run Target and activate Run Target identified by this parameter.
    ...
    /// @error score::mw::lifecycle::ExecErrc::kFailedUnexpectedTerminationOnExit if Unexpected Termination of a Process
    /// assigned to the previously active Run Target happened.
    /// @error score::mw::lifecycle::ExecErrc::kFailedUnexpectedTerminationOnEnter if Unexpected Termination of a
    /// Process assigned the requested Run Target happened.
    /// @error score::mw::lifecycle::ExecErrc::kInvalidArguments if argument passed doesn't appear to be valid (e.g.
    /// after a software update, given Run Target doesn't exist anymore)
    ...
    /// @error score::mw::lifecycle::ExecErrc::kInTransitionToSameState if there is already an ongoing request to
    /// activate requested Run Target
    /// @error score::mw::lifecycle::ExecErrc::kInvalidTransition if activation of the requested Run Target is
    /// prohibited (e.g. Off Run Target)
